...
class Building:
    ...
    def fire_alarm(self):
        print("FIRE ALARM is activated!!")
        for i in range(len(self.elevators)):
            if self.elevators[i].current_floor != self.bottom_floor:
                print(f"Elevator {i + 1} is going to floor {self.bottom_floor}.")
                self.elevators[i].go_to_floor(self.bottom_floor)
            else:
                print(f"Elevator {i + 1} is already on floor {self.bottom_floor}")
